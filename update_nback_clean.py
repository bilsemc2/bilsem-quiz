...
# Remove the banner import
content = content.replace('import GameFeedbackBanner from "./shared/GameFeedbackBanner";', '')

# Extract up to the start of the return or `if (phase === 'welcome')` block.
# We will match `if (phase === 'welcome') {` and replace everything from there to the end.
match = re.search(r"if\s*\(phase\s*===\s*'welcome'\)\s*\{", content)
if not match:
    print("Could not find start index")
    exit(1)

front = content[:match.start()]
...
with open(file_path, 'w') as f:
    f.write(front + new_jsx)

print("Updated NBackGame successfully with correct structure!")
