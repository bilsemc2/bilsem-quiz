...
import { POSSIBLE_TARGET_DYADS, TargetDyad } from './data/targetNotes';
...
const DoubleNotePage: React.FC = () => {
    ...
    const [currentTargetDyad, setCurrentTargetDyad] = useState<TargetDyad | null>(null);
    ...
    useEffect(() => {
        if (POSSIBLE_TARGET_DYADS.length > 0) {
            const randomIndex = Math.floor(Math.random() * POSSIBLE_TARGET_DYADS.length);
            setCurrentTargetDyad(POSSIBLE_TARGET_DYADS[randomIndex]);
        }
    ...
    const playTargetDyad = useCallback(() => {
        if (!currentTargetDyad || !isSamplerReady) return;
        ...
        const [note1, note2] = currentTargetDyad.notes;
        const duration = currentTargetDyad.duration;
    ...
    }, [getAudioContext, playNote, isPlayingTarget, currentTargetDyad, isSamplerReady]);

    const analyzeAudio = useCallback(async (audioBlob: Blob) => {
        if (!currentTargetDyad) return;
        setIsAnalyzing(true);
        const audioContext = getAudioContext();
        if (!audioContext) return;
    ...
    }, [getAudioContext, currentTargetDyad]);
    ...
    const handleNext = () => {
        if (analysisResult) {
            const result: RoundResult = {
                round: currentRound,
                targetNotes: currentTargetDyad?.notes.join(', ') || '',
        ...
        if (currentRound < DOUBLE_NOTE_ROUNDS) {
            setCurrentRound(prev => prev + 1);
            setRestartCounter(prev => prev + 1);
        } else {
            completeTest('double-note');
            ...
            saveResult('double-note', {
                score: (finalResults.filter(r => r.match).length / DOUBLE_NOTE_ROUNDS) * 100,
                totalRounds: DOUBLE_NOTE_ROUNDS,
                correctCount: finalResults.filter(r => r.match).length,
                details: finalResults
            });

            navigate('/atolyeler/muzik/triple-note');
        }
    };

    return (
        <div className="muzik-section-box">
            <h2 className="text-4xl font-black mb-10 text-center bg-gradient-to-r from-emerald-600 to-indigo-600 bg-clip-text text-transparent font-poppins tracking-tight">
                Çift Ses Tekrarı
            </h2>
            {showStory ? (
                <div className="max-w-2xl mx-auto">
                    {storyStep === 1 ? (
                        <StoryScreen
                            ...
                            text="Bilsem Müzik Sınavı - Çift Ses Tekrarı"
                            onPlayAudio={() => { if (!introAudio.current) introAudio.current = new Audio('/ses/4_.mp3'); introAudio.current.play(); }}
                            onContinue={() => setStoryStep(2)}
                        />
                    ) : (
                        <StoryScreen
                            imageUrl={storyImageUrl}
                            text="Şimdi iki ses duyacaksın. Bu sesleri sırayla 'na' veya 'la' hecesiyle tekrar et."
                            onPlayAudio={() => { if (!instructionAudio.current) instructionAudio.current = new Audio('/ses/4.mp3'); instructionAudio.current.play(); }}
                ...
                <div className="max-w-4xl mx-auto pb-12">
                    <ProgressIndicator currentRound={currentRound} totalRounds={DOUBLE_NOTE_ROUNDS} />
                    ...
                    <div className="flex flex-col items-center gap-6 my-10 p-10 bg-white/40 dark:bg-slate-800/20 backdrop-blur-2xl rounded-[2.5rem] border border-white/30 dark:border-white/5 shadow-2xl">
                        {currentTargetDyad ? (
                            <div className="text-center">
                                <span className="text-xs font-bold text-emerald-600 dark:text-emerald-400 uppercase tracking-[0.2em] mb-2 block">Hedef Sesler</span>
                                <div className="text-6xl font-black text-slate-800 dark:text-white mt-1 drop-shadow-sm font-poppins">
                                    {currentTargetDyad.notes.join(' & ')}
                                </div>
                            </div>
                        ...
                        <div className="flex flex-wrap justify-center gap-4 mt-4">
                            ...
                            >
                                {isPlayingTarget ? 'Çalınıyor...' : '1. Hedef Sesleri Dinle'}
                            </button>
                    ...
                    {analysisResult && !isAnalyzing && (
                        <div className={`p-6 rounded-2xl border ${analysisResult.match ? 'bg-green-500/10 border-green-500/20 text-green-700' : 'bg-red-500/10 border-red-500/20 text-red-700'} mb-6`}>
                            ...
                            <button onClick={handleNext} className="mt-4 py-2 px-6 bg-indigo-600 text-white rounded-xl font-bold hover:bg-indigo-700 transition-all">
                                {currentRound < DOUBLE_NOTE_ROUNDS ? 'Sonraki Çift Ses' : 'Testi Tamamla'}
                            </button>
                        </div>
                    )}
                </div>
