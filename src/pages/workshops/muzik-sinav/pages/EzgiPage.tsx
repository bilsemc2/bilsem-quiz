/**
 * Ezgi Tekrarı — Melody Repetition Test (20 points)
 * 4 questions × 5 points. Tactile Cyber-Pop aesthetic.
 */
...
export default function EzgiPage() {
    ...
    const handleMicToggle = useCallback(async () => {
        if (mic.isListening) {
            ...
            const accuracy = calculateMelodyAccuracy(targetMelody, mic.capturedNotes);
            ...
            if (newQ >= TOTAL_QUESTIONS) {
                submitModuleScore('ezgi', newScore, `${newQ} ezgiden ${newScore} puan`);
            }
            mic.resetCapture();
        } else {
            mic.resetCapture();
        ...
        <div className="min-h-screen bg-gray-50 dark:bg-slate-900 py-12 px-6 relative overflow-hidden transition-colors duration-300">
            <div className="max-w-3xl mx-auto space-y-8 relative z-10">
                <div className="bg-white dark:bg-slate-800 p-8 md:p-10 border-3 border-black/10 rounded-2xl shadow-neo-md relative overflow-hidden">
                    <div className="absolute top-0 left-0 w-full h-4 bg-cyber-emerald border-b-2 border-black/10" />
                    <div className="mt-4">
                        ...
                        <h1 className="text-4xl md:text-5xl font-nunito font-extrabold text-black dark:text-white uppercase tracking-tight leading-none">
                            Ezgi <span className="text-cyber-emerald">Tekrarı</span>
                        </h1>
                        <p className="text-slate-600 dark:text-slate-400 font-nunito font-bold text-lg mt-2">
                            AI bir melodi oluşturur ve piyanodan çalar — sen dinle ve tekrar et.
                        </p>
                    </div>
                </div>
                ...
                <div className="flex items-center gap-2">
                    {Array.from({ length: TOTAL_QUESTIONS }).map((_, i) => (
                        <div key={i} className={`h-2.5 flex-1 rounded-full border border-black/5 transition-all duration-500 ${i < state.currentQuestion
                            ? results[i]?.points >= 4 ? 'bg-cyber-emerald' : results[i]?.points >= 2 ? 'bg-cyber-gold' : 'bg-cyber-pink/40'
                            : i === state.currentQuestion && state.phase !== 'intro' ? 'bg-cyber-emerald animate-pulse' : 'bg-gray-200 dark:bg-slate-700'
                            }`} />
                    ))}
                </div>
                ...
                <AnimatePresence mode="wait">
                    {state.phase === 'intro' && !completed && (
                        <motion.div key="intro" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }}
                            ...
                            <div className="w-20 h-20 mx-auto bg-cyber-emerald/10 border-2 border-cyber-emerald/20 rounded-2xl flex items-center justify-center text-4xl">🎼</div>
                            <div className="space-y-2">
                                ...
                                <p className="text-slate-500 dark:text-slate-400 font-nunito font-bold text-sm leading-relaxed max-w-sm mx-auto">
                                    4 notalık bir melodi çalınır. Dinledikten sonra mikrofona basıp aynı melodiyi seslendir.
                                </p>
                            </div>
                            <button onClick={startQuestion}
                                className="inline-flex items-center gap-2 px-8 py-3 bg-cyber-emerald text-black border-2 border-black/10 font-nunito font-extrabold text-sm uppercase tracking-widest rounded-xl shadow-neo-sm hover:-translate-y-1 hover:shadow-neo-md transition-all">
                    ...
                    {state.phase === 'playing' && (
                        <motion.div key="playing" initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                            ...
                            <div className="flex justify-center gap-3 mb-4">
                                {targetMelody.map((note, i) => (
                                    <motion.div key={i} initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ delay: i * 0.2 }}
                                        className="w-14 h-14 bg-cyber-emerald/10 border-2 border-cyber-emerald/20 rounded-xl flex items-center justify-center">
                                        <span className="font-nunito font-extrabold text-cyber-emerald text-sm">{note}</span>
                                    </motion.div>
                                ))}
                            </div>
                            <p className="text-cyber-emerald font-nunito font-extrabold text-sm uppercase tracking-widest animate-pulse">Melodiyi dinle...</p>
                        </motion.div>
                    )}
                        ...
                        <motion.div key="recording" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
                            ...
                            <div className="text-center">
                                <span className="text-slate-400 font-nunito font-extrabold text-xs uppercase tracking-widest">
                                    Ezgi {state.currentQuestion + 1} / {TOTAL_QUESTIONS}
                                </span>
                                <div className="mt-3 flex justify-center gap-2">
                                    {targetMelody.map((note, i) => (
                                        <span key={i} className="px-3 py-1 bg-cyber-emerald/10 border border-cyber-emerald/20 rounded-lg font-nunito font-extrabold text-sm text-cyber-emerald">{note}</span>
                                    ))}
                                </div>
                            </div>
                                    ...
                                    <div className="flex gap-2 flex-wrap justify-center">
                                        {mic.capturedNotes.map((n, i) => (
                                            <motion.span key={i} initial={{ scale: 0 }} animate={{ scale: 1 }}
                                                className="px-2 py-1 bg-cyber-blue/10 border border-cyber-blue/20 rounded-md font-nunito font-extrabold text-xs text-cyber-blue">{n}</motion.span>
                                        ))}
                                    </div>
                                )}
                            </div>
                            ...
                            {results.length > 0 && (() => {
                                ...
                                return (
                                    <div className="space-y-4">
                                        <div className="text-5xl">{last.points >= 4 ? '🎉' : last.points >= 2 ? '👍' : '💪'}</div>
                                        <div className="text-black dark:text-white font-nunito font-extrabold text-lg">Doğruluk: %{last.accuracy}</div>
                                        <div className={`inline-block px-4 py-2 border-2 rounded-xl font-nunito font-extrabold text-sm ${last.points >= 4 ? 'bg-cyber-emerald/10 border-cyber-emerald/30 text-cyber-emerald' : last.points >= 2 ? 'bg-cyber-gold/10 border-cyber-gold/30 text-cyber-gold' : 'bg-cyber-pink/10 border-cyber-pink/30 text-cyber-pink'
                                            }`}>+{last.points} puan</div>
                                    </div>
                                );
                            })()}
                            <p className="text-slate-400 font-nunito font-extrabold text-sm">Toplam: <span className="text-black dark:text-white">{state.score}/20</span></p>
                            {state.currentQuestion < TOTAL_QUESTIONS ? (
                                <button onClick={startQuestion}
                                    className="inline-flex items-center gap-2 px-6 py-3 bg-cyber-emerald/10 border-2 border-cyber-emerald/20 text-cyber-emerald font-nunito font-extrabold text-sm uppercase tracking-widest rounded-xl hover:-translate-y-0.5 transition-all">
                                    <ArrowRight className="w-4 h-4" strokeWidth={2.5} /> Sonraki Ezgi
                                </button>
                            ) : (
