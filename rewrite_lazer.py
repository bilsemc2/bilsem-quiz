...
import { Crosshair, Eye, Sparkles, Brain, Star, Zap } from "lucide-react";
            ...
            {phase === "playing" && path.length > 0 && (
              ...
              >
                <motion.div
                  ...
                  className={`mb-6 flex items-center gap-3 px-6 py-3 rounded-2xl font-syne font-black uppercase tracking-widest text-sm border-4 border-black shadow-[6px_6px_0_#000] rotate-1 ${
                    localPhase === "preview"
                ...
                >
                  {localPhase === "preview" ? (
                    ...
                    feedbackState?.correct ? (
                      <>
                        <Star size={22} className="text-black fill-black" /> Doğru!
                      </>
                    ) : (
                      <>
                ...
                >
                  <svg
                    className="absolute inset-0 w-full h-full pointer-events-none z-10 p-6"
                    viewBox="0 0 100 100"
                  ...
                  >
                    ...
                    {localPhase === "preview" && previewSvgPath && (
                      <path d={previewSvgPath} stroke="#10b981" strokeWidth="3" fill="none" filter="url(#laserGlow)" strokeLinecap="round" strokeLinejoin="round" />
                    )}

                    {(localPhase === "playing" || !!feedbackState) && userSvgPath && (
                      <path d={userSvgPath} stroke={isWrongFeedback ? "#ef4444" : "#10b981"} strokeWidth="3" fill="none" filter="url(#laserGlow)" strokeLinecap="round" strokeLinejoin="round" />
                    )}
                  </svg>
                  ...
                  >
                    {Array.from({ length: config.gridSize * config.gridSize }).map((_, i) => {
                      ...
                      if (active) {
                        if (isWrongFeedback) {
                          nodeStyle = { background: "#ff2745", border: "4px solid #000", boxShadow: "6px 6px 0 #000" };
                        } else {
                          nodeStyle = { background: "#14f195", border: "4px solid #000", boxShadow: "6px 6px 0 #000" };
                        }
                      }
                      if (isHead) {
                        nodeStyle = { ...nodeStyle, background: isWrongFeedback ? "#ff2745" : "#fff", boxShadow: "8px 8px 0 #000", transform: "scale(1.3)" };
                      }
                      ...
                      return (
                        <div key={`${r}-${c}`} onClick={() => handleCellClick(r, c)} className="flex items-center justify-center cursor-pointer group min-w-[40px] min-h-[40px]">
                          <motion.div
                            ...
                            style={nodeStyle}
                            whileHover={localPhase === "playing" ? { scale: 1.4, boxShadow: "0 0 16px rgba(16, 185, 129, 0.6)" } : {}}
...
with open("/Users/yetenekvezeka/bilsemc2/bilsem-quiz/src/components/BrainTrainer/LazerHafizaGame.tsx", "w") as f:
    f.write(content)
