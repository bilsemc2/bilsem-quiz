  ...
  const handleAnswer = useCallback(
    (userAnswer: boolean) => {
      ...
      const t = setTimeout(() => {
        ...
        if (correct) {
          nextLevel();
          // generateRound will be triggered on next effect because we set round to null?
          // actually let's manually generate it or just let the effect handle it.
          setRound(generateRound(level + 1));
        } else {
          // If lost life and still playing, regenerate? The original game generates a new round on both, but level doesn't change.
          setRound(generateRound(level));
        }
      }, 1000);
      timeoutsRef.current.push(t);
    },
    [round, phase, feedbackState, level, playSound, showFeedback, dismissFeedback, addScore, loseLife, nextLevel],
  );
          ...
          <AnimatePresence mode="wait">
            {!feedbackState && round && (
              <motion.div
                ...
                className="w-full"
              >
                <div className="grid grid-cols-1 md:grid-cols-12 gap-8 sm:gap-12 items-center mb-8 sm:mb-12">
                  {/* Target Section */}
                  <div className="md:col-span-4 h-full">
                    <div className="bg-white dark:bg-slate-800 rounded-[3rem] border-4 border-black p-6 sm:p-10 flex flex-col items-center justify-center relative shadow-[16px_16px_0_#000] -rotate-1 h-full min-h-[250px]">
                      <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2">
                        <span className="bg-cyber-purple text-white px-6 py-2 rounded-full text-sm font-syne font-black uppercase tracking-widest border-4 border-black shadow-[4px_4px_0_#000] whitespace-nowrap">
                          HEDEF SEMBOL
                        </span>
                      </div>
                      ...
                      <motion.div
                        ...
                        className="w-32 h-32 sm:w-40 sm:h-40 bg-cyber-yellow border-8 border-black rounded-[2.5rem] flex items-center justify-center shadow-[4px_4px_0_#000] rotate-2 mt-4"
                      >
                        {(() => {
                          const TargetIcon = round.target.component;
                          return (
                            <TargetIcon
                              className="w-20 h-20 sm:w-24 sm:h-24 text-black"
                              strokeWidth={2.5}
                            />
                          );
                        })()}
                      ...
                      <p className="mt-6 text-xl sm:text-2xl font-black font-syne text-black dark:text-white bg-slate-100 dark:bg-slate-700 px-6 py-2 rounded-xl border-4 border-black shadow-[4px_4px_0_#000] rotate-1">
                        {round.target.name}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="md:col-span-8 h-full">
                    <div className="bg-slate-50 dark:bg-slate-700/50 rounded-[3rem] border-4 border-black p-6 sm:p-10 flex flex-col items-center justify-center relative shadow-[16px_16px_0_#000] rotate-1 h-full min-h-[300px]">
                      <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2">
                        <span className="bg-cyber-blue text-white px-6 py-2 rounded-full text-sm font-syne font-black uppercase tracking-widest border-4 border-black shadow-[4px_4px_0_#000] whitespace-nowrap">
                          ARAMA GRUBU
                        </span>
                      </div>

                      <div className="flex flex-wrap justify-center gap-4 sm:gap-6 mt-6 max-w-3xl">
                        <AnimatePresence mode="popLayout">
                          {round.group.map((icon, idx) => {
                            const IconComp = icon.component;
                            // Cyber-pop varying styles for items
                            const bgColors = [
                              "bg-white",
                              "bg-cyber-pink",
                              "bg-cyber-yellow",
                            ...
                            let itemClass = `w-16 h-16 sm:w-20 sm:h-20 ${bgColor} ${textColor} rounded-2xl border-4 border-black shadow-[4px_4px_0_#000] flex items-center justify-center ${rot}`;
                ...
                <div className="grid grid-cols-2 gap-4 sm:gap-8 max-w-2xl mx-auto">
                  <motion.button
                    ...
                    className="py-8 sm:py-10 rounded-[2.5rem] font-syne font-black text-2xl sm:text-4xl uppercase tracking-widest border-4 border-black transition-all bg-white dark:bg-slate-800 text-black dark:text-white shadow-[12px_12px_0_#000] hover:-translate-y-2 hover:-translate-x-2 hover:shadow-[16px_16px_0_#000] -rotate-2"
                  >
                    <div className="flex flex-col items-center">
                      <XCircle size={40} className="mb-2" strokeWidth={3} />
                      <span>YOK</span>
                      <span className="text-xs opacity-60 mt-2 hidden sm:block font-chivo tracking-normal">
                        ← Sol Ok
                      </span>
                    </div>
                  </motion.button>
                  ...
                  <motion.button
                    ...
                    disabled={!!feedbackState}
                    className="py-8 sm:py-10 rounded-[2.5rem] font-syne font-black text-2xl sm:text-4xl uppercase tracking-widest border-4 border-black transition-all bg-cyber-green text-black shadow-[12px_12px_0_#000] hover:-translate-y-2 hover:-translate-x-2 hover:shadow-[16px_16px_0_#000] rotate-2"
                  >
                    <div className="flex flex-col items-center">
                      <CheckCircle2 size={40} className="mb-2" strokeWidth={3} />
                      <span>VAR</span>
                      <span className="text-xs opacity-60 mt-2 hidden sm:block font-chivo tracking-normal">
                        Sağ Ok →
                      </span>
...
with open("/Users/yetenekvezeka/bilsemc2/bilsem-quiz/src/components/BrainTrainer/SymbolSearchGame.tsx", "w") as f:
    f.write(content)
